...
export default class Checkout extends Component {
  ...
  getAdminCharge =async()=> {
    ...
    const getData= this.ordercounters.collection('charges').doc('delivery_charge');
        ...
        <TearLines  ref="top"/>
            <View style={styles.invoice}  onLayout={(e) => {
                    ...
                    <View>
                        ...
                        <Grid  style={{padding: 8}}>
                            <Col>
                                <Text style={{fontSize: 13,  color:'green'}}>Extra Kilometer Charge {this.state.summary==undefined? null: '( '+Math.round(this.state.summary.distance/1000*10)/10}km from store)</Text>
                            </Col>
                            <Col>
                  <Text style={{textAlign: 'right', fontSize: 13 ,color: 'green'}}>₱{Math.round(this.extraKMCharges())}</Text>            
                            </Col>
                        </Grid>
