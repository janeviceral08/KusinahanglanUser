                   ...
                   <View style={{width:width,marginRight: 0,backgroundColor:'white', flexDirection:'row', borderBottomWidth:2, borderColor:"#cccccc",paddingHorizontal: 10}}>
                     ...
                     <View style={{flex:1, backgroundColor:'transparent', padding:10, justifyContent:"space-between"}} >                    
                       ...
                       <View style={{flexDirection:'row',justifyContent:'space-between'}}>
                         ...
                         {item.sale_price ? [item.addons == null  || item.addons.length < 1? <Text style={{fontWeight:'bold',color:"#33c37d",fontSize:15}}>₱{(item.sale_price*item.qty).toFixed(2)}</Text> :<Text style={{fontWeight:'bold',color:"#33c37d",fontSize:15}}>₱{((item.sale_price*item.qty)+(item.total_addons*item.qty)).toFixed(2)}</Text>] : [item.addons == null || item.addons.length < 1 ? <Text style={{fontWeight:'bold',color:"#33c37d",fontSize:15}}>₱{(item.price*item.qty).toFixed(2)}</Text> : <Text style={{fontWeight:'bold',color:"#33c37d",fontSize:15}}>₱{((item.price*item.qty)+(item.total_addons*item.qty)).toFixed(2)}</Text>]}
