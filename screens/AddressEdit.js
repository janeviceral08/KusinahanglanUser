...
updateTextInput = (text, field) => {
 ...
 async onEditSave(){
    ...
    addressRef.doc(userId).get().then(snapshot => {
      ...
      updatedCart[itemIndex]['lat'] = this.state.userPoint.latitude;  
      updatedCart[itemIndex]['long'] = this.state.userPoint.longitude;  
 ...
 async onCreateAddress() {
  ...
  render() {
    ...
    />
            ...
            <Button onPress={this.myCurrentLocation} style={{ height: 30, backgroundColor:  "salmon", marginTop: 10, justifyContent: 'center', alignContent: 'center'}}>
              <Text style={[styles.textSign, { color:'#fff'}]}>Get your Location</Text></Button>
                  ...
                  <Input placeholder={this.state.selectedCity} value={this.state.selectedCity}   onChangeText={(text) => this.fetchBarangay(text)} placeholderTextColor="#687373" />
                  
                  </Item>
                        
                   <Text style={{marginTop: 15, fontSize: 10}}>Barangay</Text>
                  <Item>
                   <Input placeholder={this.state.selectedBarangay} value={this.state.selectedBarangay}   onChangeText={(text) => this.setState({selectedBarangay: text})} placeholderTextColor="#687373" />
